...
if (typeof String.prototype.base64Encode == 'undefined') {
	String.prototype.base64Encode = function () {
		if (typeof Base64 != 'undefined') return Base64.btoa(this);
...
if (typeof String.prototype.base64Decode == 'undefined') {
	String.prototype.base64Decode = function () {
		if (typeof Base64 != 'undefined') return Base64.atob(this);
...
function CRYPTOTEST() {
	...
	const pass = "123"
	const n = 256
	const encrypted = Aes.Ctr.encrypt(x, pass, n)
	const decrypted = Aes.Ctr.decrypt(encrypted, pass, n)
	const badDecrypted = Aes.Ctr.decrypt(encrypted, "456", n)
	console.log({ x, pass, n, encrypted, decrypted, badDecrypted })
}

// CRYPTOTEST()

function decryptAll(data, pass, nbits) {
	const decrypt = (x) => Aes.Ctr.decrypt(x, pass, nbits)

	const config = JSON.parse(decrypt(data.config))
	const movs = JSON.parse(decrypt(data.movs))
	const originalSeries = data.series
	const series = {}
	for (const [k, v] of Object.entries(originalSeries)) {
		series[decrypt(k)] = v
	}

	return { config, movs, series }
}


const pass = process.argv[2]
const nbits = parseInt(process.argv[3])

const fs = require("fs");
const stdinBuffer = fs.readFileSync(0);
const encryptedData = JSON.parse(stdinBuffer.toString());
const data = decryptAll(encryptedData, pass, nbits)

console.log(Object.keys(data.series))
const x = data.series['uber/unitsAcc']
console.log(x)
